...
**③ SessionStart hook (매일 거울)** — `aimm init`이 SessionStart hook도 함께 등록한다(멱등). Claude Code를 열 때마다(startup · resume) `systemMessage`로 어제·이번주(최근7일) 비용-확인 세션 현황을 한 줄 표시한다. compact·clear 이벤트에는 뜨지 않는다.

```
🪞 어제 기록 없음 · 이번주(최근7일) 15세션 · $378.25
```
...
src/
  ...
  cli.ts              진입점: metrics / standup / analyze / portrait / init / hook / session-start / mcp
test/                 25개 파일, 204 테스트
```
...
**SessionStart 거울**(`aimm session-start` — 매일 Claude Code 시작 시 한 줄 현황) · CLI. **204 테스트 그린.**
