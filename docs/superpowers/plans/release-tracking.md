...
| 파일 | 책임 | 신규/수정 |
|------|------|-----------|
| `docs/releases/v0.1.0-phase-1-baseline.md` | v0.1.0 백필 상세 노트 | 신규 |
...
**Files:**
- Create: `docs/releases/v0.1.0-phase-1-baseline.md`
...
- [ ] **Step 2: 노트 파일 작성** — `docs/releases/v0.1.0-phase-1-baseline.md`를 아래 내용으로 생성한다(Step 1에서 수치가 바뀌었으면 반영):
...
Run:
```bash
test -f docs/releases/v0.1.0-phase-1-baseline.md && echo "EXISTS"
grep -E "turbo-pra|checkin-be|seoultel|supertonic|kiosk|bbibbi|arreo|mcmp|AIWS" docs/releases/v0.1.0-phase-1-baseline.md && echo "❌ 타 프로젝트명 누출" || echo "✅ 누출 없음"
```
...
```bash
git add docs/releases/v0.1.0-phase-1-baseline.md
...
test -f docs/releases/v0.1.0-phase-1-baseline.md && echo "LINK OK"
```
...
- `docs/releases/v0.1.0-phase-1-baseline.md`가 설계 §4 템플릿(백필 약식)으로 작성되고, 타 프로젝트명 누출이 없다.
